...
<head>
    ...
    <style>
        /* Estilos generales */
        :root {
            /* (OPTIMIZACIÓN MÓVIL) Altura del encabezado reducida para dar más espacio al contenido */
        ...
        .header-content-container {
             /* Altura dinámica para scroll en el content area */
            height: calc(100vh - var(--header-height-mobile));
            overflow-y: auto;
        }

        .tab-bar-sticky {
            /* Fija la barra de pestañas en móvil */
            top: calc(var(--header-height-mobile) - var(--tabs-height));
        }
        
        /* (OPTIMIZACIÓN MÓVIL) Añade un degradado visual para indicar que los botones de acceso rápido son desplazables horizontalmente */
        ...
        @media (min-width: 768px) {
            ...
            /* Estilo específico del contenedor redimensionable en DESKTOP */
            ...
            .resizable-panel {
                flex-grow: 1;
                min-width: 25%; /* Mínimo 25% de ancho para cada panel */
                max-width: 75%; /* Máximo 75% de ancho para cada panel */
        ...
        /* (NUEVO) Estilos de impresión para el certificado */
        @media print {
            body * {
                visibility: hidden;
            }
            ...
            /* Asegura que los colores de fondo se impriman (aunque no tenemos en este diseño) */
            body {
        ...
        .iframe-container {
            /* (OPTIMIZACIÓN MÓVIL) Altura de iframes reducida para mejorar la visibilidad de los paneles apilados */
            height: 350px; 
            min-height: 350px; 
        }
        @media (min-width: 768px) {
    ...
    <script>
        ...
        // URLs de incrustación optimizadas
        const EMBED_URLS = {
        ...
        // (MODIFICADO) Logo eliminado por petición del usuario
        ...
        function switchTab(tabName) {
            ...
            // Forzar scroll al inicio del contenedor de contenido (relevante para móvil)
            const mainContent = document.querySelector('main');
            if (mainContent) {
                 mainContent.scrollTop = 0;
            }

            // (NUEVO) Cargar la imagen de ejemplo si vamos a la pestaña de certificado y no hay nada
        ...
        function copySummary() {
            if (!lastGeneratedText) { alertMessage('No hay análisis generado para copiar.', 'error'); return; }
            ...
            const nextBoldIndex = summaryText.indexOf('**', 1); // Busca el siguiente doble asterisco después del inicio
            if (nextBoldIndex > -1) {
            ...
            if (cleanText) {
                ...
                alertMessage('El resumen de HubSpot está vacío.', 'error');
            }
        }
        ...
        async function runAnalysis() {
            ...
            resultsDiv.innerHTML = '<p class="text-gray-500 italic mt-3">Activando el análisis consultivo...</p>';
            lastGeneratedText = '';

            const systemInstruction = "Eres el 'Cursor de acción', un asistente experto en venta consultiva B2B y seguimiento. **Tus tips de venta consultiva se basan en los más grandes expertos de la venta B2B del mundo (como Chris Voss, Jeffrey Gitomer, Zig Ziglar, Sandler Selling, SPIN Selling y Challenger Sale).** Tu tarea es analizar una nota corta del usuario sobre una interacción o visita (especialmente en el sector inmobiliario/financiero B2B) y generar un análisis estructurado y útil con el siguiente formato, utilizando solo texto y saltos de línea:\n\n**Resumen de la Venta Consultiva:** Síntesis del estado de la oportunidad, dolor detectado y valor propuesto (usando terminología de experto B2B).\n**Siguiente Acción:** Una sugerencia de paso práctico e inmediato (Ej: Preparar propuesta B, Llamar a Director, Programar seguimiento).\n**Borrador de Correo:** Un borrador de correo electrónico breve y profesional para el seguimiento.\n**Resumen de la visita para HubSpot:** Texto conciso y directo, sin encabezados, ideal para copiar y pegar como nota de actividad en el CRM (máximo 3 líneas, sin formato Markdown).";
                ...
                try {
                    ...
                    if (text) {
                        ...
                        resultsDiv.innerHTML = `
                            ...
                            <button onclick="copySummary()" class="px-4 py-2 text-sm font-semibold rounded-lg text-white bg-green-600 hover:bg-green-700 transition duration-200 shadow-md w-full sm:w-auto">📋 Copiar Resumen HubSpot</button>
                        `;
                    } else {
                        resultsDiv.innerHTML = '<p class="text-red-600 font-semibold">Error: No se pudo generar el análisis. Inténtalo de nuevo.</p>';
                    }
                    break;
                } catch (error) {
                    console.error("Analysis Error:", error);
                    if (i === MAX_RETRIES - 1) {
                        resultsDiv.innerHTML = '<p class="text-red-600 font-semibold">Error: Falló la conexión con el asistente de Gemini después de varios intentos.</p>';
                    }
                } // <!-- Cierra catch -->
            } // <!-- Cierra for loop -->
        ...
        async function runMarketAnalysis() {
            ...
            const systemInstruction = "Eres un analista experto en el mercado inmobiliario español. Tu tarea es proporcionar un análisis conciso y actualizado de la zona proporcionada por el usuario. Basa tus respuestas en información reciente de la web. Estructura la respuesta de la siguiente manera:\n\n**Tendencias del Mercado:** Un párrafo sobre la situación actual (precios, demanda).\n**Precios de Alquiler (Promedio):**\n- 1 Habitación: [Precio]\n- 2 Habitaciones: [Precio]\n- 3 Habitaciones: [Precio]\n**Perfil de la Zona:**\n- **Servicios Clave:** (Transporte, colegios, parques)\n- **Demografía:** (Tipo de residentes, ambiente)\n\nUtiliza un tono profesional y directo. Finaliza siempre listando las fuentes consultadas.";
            ...
            for (let i = 0; i < MAX_RETRIES; i++) {
                try {
                    ...
                    if (text) {
                        ...
                        resultsDiv.innerHTML = '<p class="text-red-600 font-semibold">Error: No se pudo generar el análisis de mercado.</p>';
                    }
                    break;
                } catch (error) {
            ...
            button.disabled = false;
            button.innerHTML = '✨ Obtener Análisis de Mercado';
        }

        // --- (NUEVO) Lógica de la Calculadora de Precios ---
        function calculatePrice() {
            ...
            // Deshabilitar botón si no hay alquiler
            ...
            if (!PRICING_RATES[payerType] || !PRICING_RATES[payerType][propertyType]) {
                alertMessage('Configuración de precios no válida.', 'error');
                resultsContainer.style.display = 'none';
                return;
            }
        ...
        // --- (NUEVO) Lógica del Generador de Certificados ---
        ...
        function handleScreenshotUpload(event) {
            ...
            if (file && file.type.startsWith('image/')) {
                ...
                reader.onload = function(e) {
                    ...
                    generateCertificate(); // Generar automáticamente al subir
                };
                reader.readAsDataURL(file);
            } else {
                uploadedScreenshotData = null;
                alertMessage('Por favor, sube un archivo de imagen válido.', 'error');
            }
        }
            ...
            if (lastCalculatedPriceString && lastCalculatedPercentageString) {
                priceHtmlBlock = `
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-700 mb-3">Coste de la Garantía (Pago Único):</h3>
            ...
            if (!uploadedScreenshotData) {
                ...
                previewDiv.innerHTML = `
                    <div id="certificate-output" class="border border-gray-200 rounded-lg p-6 sm:p-8 max-w-3xl mx-auto bg-white shadow-lg">
                        <!-- Encabezado Corporativo (ELIMINADO POR PETICIÓN DEL USUARIO) --><!--
                        <div class="flex justify-between items-center border-b pb-4 mb-6">
                            <h2 class="text-2xl font-bold text-indigo-700">SDA Finaer</h2>
                            <span class="text-xl font-semibold text-gray-800">Certificado de Aprobación</span>
                        </div>
                        --><!-- Imagen de la Captura --><div class="mb-6">
                            <!-- <h3 class="text-lg font-semibold text-gray-700 mb-3">Solicitud Aprobada:</h3> (Título opcional eliminado para look más limpio) --><img id="screenshot-preview-image" src="https://i.imgur.com/gYf20fV.png" alt="Captura de pantalla de la solicitud" class="w-full rounded-lg border border-gray-200 shadow-sm">
                        </div>
                        
                        <!-- (MODIFICADO) Pasos a Seguir con Estilo Corporativo --><div>
                            <h3 class="text-xl font-bold text-gray-800 mb-5 border-b pb-3">Para proceder con la firma de la garantía, necesitamos que nos facilites la siguiente información:</h3>
                            
                            <!-- (MODIFICADO) Lista con iconos y sin títulos --><ul class="space-y-4 text-gray-700">
                                <li class="flex items-start gap-3">
                                    ...
                                    <span class="block text-sm">Correos electrónicos y números de móviles de inquilinos y propietario (todos los propietarios).</span>
                                </li>
                                <li class="flex items-start gap-3">
                                    ...
                                    <span class="block text-sm">Dirección completa del inmueble, calle, portal, planta, puerta, ciudad, provincia, código postal.</span>
                                </li>
                                <li class="flex items-start gap-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="flex-shrink-0 text-red-600 mt-0.5"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/><path d="M8 14h.01"/><path d="M12 14h.01"/><path d="M16 14h.01"/><path d="M8 18h.01"/><path d="M12 18h.01"/><path d="M16 18h.01"/></svg>
                                    <span class="block text-sm">Número de DNI y nombre completos de propietarios.</span>
                                </li>
                                <li class="flex items-start gap-3">
                                ...
                                <li class="flex items-start gap-3">
	                                ...
	                                <span class="block text-sm">Que me especifiques si lo quieres pagar en 2, 3 ó 6 cuotas sin intereses.</span>
                                </li>
                                 <li class="flex items-start gap-3">
                                    ...
                                    <span class="block text-sm">¿Firma telemática o presencial?</span>
                                </li>
                            </ul>
                ...
                `;
                printButton.style.display = 'none'; // Ocultar botón de imprimir para el ejemplo
                uploadHint.style.display = 'block'; // Mostrar hint de subir
            } else {
                // Si SÍ hay imagen subida, la usamos
                const previewImage = document.getElementById('screenshot-preview-image');
                if (previewImage) {
                    previewImage.src = uploadedScreenshotData;
                }
                printButton.style.display = 'inline-flex'; // Mostrar botón de imprimir
        ...
        function printCertificate() {
            ...
            // (NUEVO) Lista de requisitos (más robusto que parsear HTML)
            const requirements = [
                "Correos electrónicos y números de móviles de inquilinos y propietario (todos los propietarios).",
                "Dirección completa del inmueble, calle, portal, planta, puerta, ciudad, provincia, código postal.",
                "Número de DNI y nombre completos de propietarios.",
                "IBAN para domiciliar la factura de Finaer y nombre del titular de la cuenta.",
                "Que me especifiques si lo quieres pagar en 2, 3 ó 6 cuotas sin intereses.",
                "¿Firma telemática o presencial?"
            ];

            // (NUEVO) Generar bloque de precio para impresión si existe
            let pricePrintHtml = '';
            if (lastCalculatedPriceString && lastCalculatedPercentageString) {
                pricePrintHtml = `
                    <h3 style="font-size: 1.1rem; font-weight: 600; color: #3f3f46; margin-top: 1rem; margin-bottom: 0.5rem; border-top: 1px solid #e4e4e7; padding-top: 1rem;">Coste de la Garantía (Pago Único):</h3>
            ...
            // (NUEVO) CSS Corporativo para la impresión (sin Tailwind)
            ...
            newWindow.document.write(`
                ...
                .page-container {
                    ...
                    margin: 0 auto; /* Sin márgenes verticales para evitar página en blanco */
                    padding: 1.5rem;
                ...
                .print-header {
                    ...
                    border-bottom: 6px solid #dc2626; /* border-red-600, más grueso */
                    /* (MODIFICADO) Márgenes reducidos para compactar */
                    padding-bottom: 0; /* Sin padding */
                    margin-bottom: 1rem;
                }
                .print-header .title h1 {
                    /* (MODIFICADO) Tamaño reducido */
                    font-size: 2rem;
                    font-weight: 800;
                    color: #18181b;
                    margin: 0;
                ...
                */
                /* (NUEVO) Estilo para el subtítulo/introducción a la lista */
                .print-content h3 {
                    font-size: 1.1rem; /* Más pequeño que el h2 anterior */
                ...
                .requirements-list li {
                    ...
                    gap: 0.75rem;
                    /* (MODIFICADO) Márgenes y padding reducidos para compactar */
                    margin-bottom: 0.5rem;
                    color: #3f3f46; /* text-zinc-700 */
                    padding: 0.5rem;
                ...
                /* (ELIMINADO) Pie de página y sus estilos */
            `);
            ...
            newWindow.document.write(`
                <div class="page-container">
                    <div class="print-header">
                        <!-- (MODIFICADO) Logo eliminado. Esta div ahora solo actúa como la barra de color superior. -->
                    </div>
                    
                    <div class="print-content">
                        <!-- (MODIFICADO) Título h2 eliminado --><img src="${imgSrc}" alt="Captura de pantalla de la solicitud aprobada">
                        
                        <!-- (MODIFICADO) Título h2 reemplazado por h3 con el nuevo texto --><h3>Para proceder con la firma de la garantía, necesitamos que nos facilites la siguiente información:</h3>
                        <ul class="requirements-list">
                            ${requirements.map(req => `<li>${req}</li>`).join('')}
                        </ul>
                        
                        <!-- (NUEVO) Precio Inyectado para Impresión -->
                        ${pricePrintHtml}
                    </div>
                    
                    <!-- (MODIFICADO) Pie de página eliminado -->
                </div>
            `);
            ...
            // para asegurar que el DOM de la nueva ventana esté listo.
            setTimeout(() => {
                ...
                try {
                    ...
                    // Esto da tiempo al servicio de impresión/PDF del móvil (Android/iOS)
        ...
        // --- To-Do List Logic (simplificado sin Firebase por la restricción de 1 archivo) ---
        let tasks = [];
        ...
        function renderTasks() {
            ...
            if (tasks.length === 0) {
                listContainer.innerHTML = '<p class="text-gray-500 text-center py-4">No hay tareas pendientes. ¡Añade una!</p>';
                return;
            }
        ...
        // NUEVO: Función para tachar/destachar paradas de la ruta
        ...
        function generateRoute(mode) {
            ...
            // Añadir parámetro de modo de viaje
            const travelMode = mode === 'w' ? 'w' : 'r'; // 'w' para caminar, 'r' para transporte público
            ...
            const modeText = mode === 'w' ? 'a Pie' : 'en Transporte Público';
            resultsDiv.innerHTML = `
                <div class="p-4 bg-green-50 border border-green-200 rounded-lg text-center animate-fade-in-down">
                    ...
                    <p class="text-sm text-green-700 mb-3">Haz clic en el botón para abrir la ruta en Google Maps.</p>
            ...
            `;
            alertMessage('Ruta generada con éxito.', 'success');
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (monthlyRentInput && payerTypeSelect && propertyTypeSelect && calculateButton) {
                ...
                // También al hacer clic en el botón (aunque se actualiza en vivo)
    ...
    <div class="fixed inset-0 flex flex-col">
        <!-- HEADER COMPACTO Y FIJO --><header class="bg-gray-800 text-white shadow-lg z-20 flex-shrink-0" style="height: var(--header-height-mobile);">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-0">
                <!-- (OPTIMIZACIÓN MÓVIL) Margen inferior reducido --><h1 class="text-xl sm:text-3xl font-extrabold tracking-tight text-white mb-1 sm:mb-2">
                    SDA <span class="text-indigo-400">Alto Rendimiento</span>
                </h1>
                
                <!-- (OPTIMIZACIÓN MÓVIL) Contenedor con pista visual de scroll --><div class="flex overflow-x-auto gap-2 sm:gap-3 pb-3 -mx-4 px-4 sm:mx-0 sm:px-0 border-b border-gray-700 scroll-hint-container">
                    <!-- (OPTIMIZACIÓN MÓVIL) Padding de botones reducido para un look más compacto --><button onclick="window.open(SAI_FINAER_URL, '_blank')" class="external-link-button flex-shrink-0 flex items-center px-2.5 py-1 text-xs sm:text-sm font-semibold rounded-lg text-white bg-red-600 hover:bg-red-700">
            ...
            <!-- Navegación de Pestañas FIJA (Sticky) --><div class="sticky tab-bar-sticky z-30 bg-gray-800 shadow-md">
                 <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex space-x-4 overflow-x-auto whitespace-nowrap pb-2">
                        ...
                        <!-- (NUEVO) Botón de Pestaña Certificado --><button class="tab-button flex items-center gap-1 py-2 px-1 text-sm" data-tab="certificado" onclick="switchTab('certificado')"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><path d="m10.5 17-2-2 2-2"/><path d="m13.5 17 2-2-2-2"/></svg>Certificado</button>
        ...
        <!-- (OPTIMIZACIÓN MÓVIL) Padding reducido en el contenedor principal para pantallas pequeñas --><main class="flex-grow overflow-y-auto p-3 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
            <!-- Contenido Pestaña 1: Vista General (Redimensionable en desktop) --><div id="general-content" class="tab-content">
                <!-- En móvil es una cuadrícula apilada (grid grid-cols-1), en desktop es un contenedor flex redimensionable (md:flex) --><div id="general-resizable-container" class="grid grid-cols-1 gap-6 md:gap-0 min-h-full">
                ...
                <!-- NUEVO: Generador de Ruta Comercial --><div class="card p-6 w-full mt-6">
                    ...
                    <p class="mb-4 text-gray-600 text-sm">Introduce una lista de direcciones (una por línea) para generar una ruta optimizada en Google Maps.</p>
                    ...
                    <div class="mt-4 flex flex-col sm:flex-row gap-3">
                        ...
                        <button onclick="generateRoute('r')" class="w-full sm:w-1/2 px-4 py-2 bg-purple-600 text-white font-semibold rounded-lg shadow-md hover:bg-purple-700 transition duration-200 flex items-center justify-center gap-2">
                            🚇 Generar Ruta en Transporte Público
                        </button>
                    </div>
            ...
            <!-- Contenido Pestaña: Análisis de Mercado --><div id="mercado-content" class="tab-content">
                <div class="card p-6 max-w-4xl mx-auto w-full">
                    <h2 class="text-2xl font-extrabold text-gray-800 mb-4 flex items-center gap-2">
                        ...
                        Análisis de Mercado <span class="font-normal text-gray-500 text-base hidden sm:inline">(Búsqueda en tiempo real)</span>
                    </h2>
                    <p class="mb-4 text-gray-600">Obtén un informe conciso y actualizado del mercado inmobiliario para cualquier zona, barrio o ciudad de España.</p>
                    ...
                    <div id="market-analysis-results" class="mt-6 text-sm text-gray-800">
                        <p class="text-gray-500">El informe de mercado aparecerá aquí.</p>
                    </div>
                </div>
            </div>

            <!-- Contenido Pestaña: Cursor de Acción --><div id="cursor-content" class="tab-content">
                <div class="card p-6 max-w-4xl mx-auto w-full">
                    <h2 class="text-2xl font-extrabold text-gray-800 mb-4 flex items-center gap-2">
                        ...
                        Cursor de acción <span class="font-normal text-gray-500 text-base hidden sm:inline">(Asistente Consultivo IA)</span>
                    </h2>
                    <p class="mb-4 text-gray-600 text-sm">Analiza notas de interacciones para obtener un resumen de venta B2B, la siguiente acción y un borrador de correo, optimizado para el CRM (HubSpot).</p>
                    <textarea id="analysis-prompt" rows="5" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-150"
                              placeholder="Ej: Visita con Inmobiliaria Élite. El director (Juan) confirmó que su dolor es la lenta aprobación de hipotecas y le propuse nuestra solución 24h. Presupuesto ok, enviar propuesta mañana."></textarea>
                    ...
                    <div id="analysis-results" class="mt-6 text-sm text-gray-800">
                        <p class="text-gray-500">El resultado del análisis aparecerá aquí.</p>
                    </div>
                </div>
            </div>

            <!-- Contenido Pestaña: Incentivos --><div id="incentivos-content" class="tab-content">
                <div class="card p-6 w-full">
                    ...
                    <!-- Tabla de Incentivos con scroll horizontal para móvil --><div class="overflow-x-auto shadow-lg rounded-lg border">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" colspan="3" class="px-6 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider border-b border-l">Renta Vivienda (€)</th>
                                    <th scope="col" colspan="3" class="px-6 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider border-b border-l">Renta Habitación (€)</th>
                                </tr>
                                <tr>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider border-l">500 - 1500 €</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider">1501 - 2500 €</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider">+ 2500 €</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider border-l">100 - 500 €</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider">501 - 1000 €</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider">+ 1000 €</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200 text-sm">
                                <tr>
                                    <td class="px-3 py-4 whitespace-nowrap font-bold text-indigo-700 sticky left-0 bg-white z-10">Zona 1</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center border-l">25€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">30€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">35€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center border-l">12€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">15€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">18€</td>
                                </tr>
                                <tr class="bg-gray-50">
                                    <td class="px-3 py-4 whitespace-nowrap font-bold text-indigo-700 sticky left-0 bg-gray-50 z-10">Zona 2</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center border-l">20€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">25€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">30€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center border-l">10€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">12€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">15€</td>
                                </tr>
                                 <tr>
                                    <td class="px-3 py-4 whitespace-nowrap font-bold text-indigo-700 sticky left-0 bg-white z-10">Zona 3</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center border-l">15€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">20€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">25€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center border-l">8€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">10€</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-gray-700 text-center">12€</td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="mt-8">
                        ...
                        <div class="space-y-3 text-sm text-gray-700 bg-gray-50 p-4 rounded-lg">
                            <p><strong>Zona 1 (Máximo Incentivo):</strong> Centro Madrid, Arganzuela, Retiro, Salamanca, Chamberí, Chamartín.</p>
                            <p><strong>Zona 2 (Incentivo Medio):</strong> Usera, Moratalaz, Carabanchel, Ciudad Lineal, Tetuán, Fuencarral-ElPardo, San Blas-Canillejas, Villa de Vallecas, Barajas.</p>
                            <p><strong>Zona 3 (Incentivo Básico):</strong> Moncloa-Aravaca, Latina, Puente de Vallecas, Hortaleza, Villaverde, Vicálvaro, Resto de la Comunidad de Madrid.</p>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Contenido Pestaña: Pisos (NUEVA ESTRUCTURA REDIMENSIONABLE) --><div id="pisos-content" class="tab-content">
                <!-- Contenedor con layout flex en desktop, y grid apilado en móvil --><div id="pisos-resizable-container" class="grid grid-cols-1 gap-6 md:gap-0 min-h-full">
            ...
            <!-- (NUEVO) Contenido Pestaña: Generador de Certificado --><div id="certificado-content" class="tab-content">
                <div class="card p-6 max-w-4xl mx-auto w-full">
                    ...
                    <div class="mb-8 border-b border-gray-200 pb-8">
                        <h2 class="text-2xl font-extrabold text-gray-800 mb-4 flex items-center gap-2">
                            ...
                            Calculadora de Precios de Garantía
                        </h2>
                        <p class="mb-6 text-gray-600 text-sm">Calcula el coste de la garantía basado en el alquiler mensual y quién contrata. El precio es un porcentaje de la anualidad.</p>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            <div>
                                <label for="precio-alquiler-mensual" class="block text-sm font-medium text-gray-700 mb-1">Alquiler Mensual (€)</label>
                        ...
                        <div id="precio-resultado-container" class="mt-6" style="display: none;">
                            <div class="calculator-result">
                                <span class="block text-sm font-medium text-gray-600 mb-1">Precio Final de la Garantía (Pago Único)</span>
                    ...
                    <h2 class="text-2xl font-extrabold text-gray-800 mb-4 flex items-center gap-2">
                        ...
                        Generador de Certificado de Aprobación
                    </h2>
                    <p class="mb-6 text-gray-600 text-sm">Utiliza la calculadora de arriba para fijar el precio. Luego, sube la captura de pantalla de la solicitud 'Aprobada' del SAI. El certificado se generará automáticamente abajo (incluyendo el precio) listo para imprimir o guardar como PDF.</p>
                    
                    <!-- Área de Carga de Archivo --><div class="file-upload-wrapper w-full mb-6">
                    ...
                    <!-- Botón de Imprimir/Guardar --><div class="text-center mb-6">
                        ...
                        <p id="upload-hint" class="text-sm text-gray-500 mt-2">Sube una imagen para activar el botón de impresión.</p>
                    </div>

                    <!-- Área de Vista Previa del Certificado --><h3 class="text-xl font-bold text-gray-800 mb-4 border-t pt-6">Vista Previa del Certificado</h3>
                    <div id="certificate-preview" class="bg-gray-100 p-4 rounded-lg">
                        <!-- El certificado generado por JS aparecerá aquí --></div>
                </div>
            </div>

            <!-- Contenido Pestaña: Spotify --><div id="spotify-content" class="tab-content">
                <div class="card p-6 max-w-4xl mx-auto w-full">
            ...
            <!-- Contenido Pestaña: To-Do List --><div id="todo-content" class="tab-content">
                <div class="card p-6 max-w-xl mx-auto w-full">
                    ...
                    <div class="flex mb-6">
                        <input type="text" id="todo-input" class="w-full p-3 border border-gray-300 rounded-l-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-150" placeholder="Añadir nueva tarea, Ej: Llamar a Juan (Élite)">
                    ...
                    <div id="todo-list" class="space-y-3">
                        <!-- Las tareas se generarán aquí --></div>
                </div>
            </div>
        </main>
    </div>
